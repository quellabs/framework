<?php
	...
	class DateUnitTest extends TestCase {
		...
		// =========================================================================
		// AstDate::tryParseInterval — single unit
		// =========================================================================
		...
		public function testSingleDayInterval(): void {
			$this->assertSame(86400, AstDate::tryParseInterval('1 day'));
		}
		
		public function testSingleDaysInterval(): void {
			$this->assertSame(86400 * 6, AstDate::tryParseInterval('6 days'));
		}
		
		public function testSingleHoursInterval(): void {
			$this->assertSame(3600 * 2, AstDate::tryParseInterval('2 hours'));
		}
		
		public function testSingleMinutesInterval(): void {
			$this->assertSame(60 * 30, AstDate::tryParseInterval('30 minutes'));
		}
		
		public function testSingleSecondsInterval(): void {
			$this->assertSame(45, AstDate::tryParseInterval('45 seconds'));
		}
		
		public function testSingleWeeksInterval(): void {
			$this->assertSame(604800 * 3, AstDate::tryParseInterval('3 weeks'));
		}
		
		public function testSingleMonthsInterval(): void {
			// 1 month = 30 days = 2592000 seconds
			$this->assertSame(2592000, AstDate::tryParseInterval('1 month'));
		}
		
		public function testSingleYearsInterval(): void {
			// 1 year = 365 days = 31536000 seconds
			$this->assertSame(31536000, AstDate::tryParseInterval('1 year'));
		}
		
		// =========================================================================
		// AstDate::tryParseInterval — singular forms
		// =========================================================================
		...
		public function testSingularSecond(): void {
			$this->assertSame(1, AstDate::tryParseInterval('1 second'));
		}
		
		public function testSingularMinute(): void {
			$this->assertSame(60, AstDate::tryParseInterval('1 minute'));
		}
		
		public function testSingularHour(): void {
			$this->assertSame(3600, AstDate::tryParseInterval('1 hour'));
		}
		
		public function testSingularWeek(): void {
			$this->assertSame(604800, AstDate::tryParseInterval('1 week'));
		}
		
		// =========================================================================
		// AstDate::tryParseInterval — composite intervals (QUEL-style)
		// =========================================================================
		...
		public function testCompositeYearsAndMinutes(): void {
			...
			$this->assertSame($expected, AstDate::tryParseInterval('4 years 20 minutes'));
		}
		
		public function testCompositeDaysAndHours(): void {
			$expected = 2 * 86400 + 6 * 3600;
			$this->assertSame($expected, AstDate::tryParseInterval('2 days 6 hours'));
		}
		
		public function testCompositeThreeComponents(): void {
			$expected = 1 * 86400 + 2 * 3600 + 30 * 60;
			$this->assertSame($expected, AstDate::tryParseInterval('1 day 2 hours 30 minutes'));
		}
		
		public function testCompositeWeeksAndDays(): void {
			$expected = 1 * 604800 + 3 * 86400;
			$this->assertSame($expected, AstDate::tryParseInterval('1 week 3 days'));
		}
		
		// =========================================================================
		// AstDate::tryParseInterval — case insensitivity
		// =========================================================================
		...
		// =========================================================================
		// AstDate::tryParseInterval — negative intervals
		// =========================================================================
		...
		public function testNegativeDays(): void {
			$this->assertSame(-86400 * 30, AstDate::tryParseInterval('-30 days'));
		}
		
		// =========================================================================
		// AstDate::tryParseInterval — "now" and non-interval strings
		// =========================================================================
		
		/**
		 * "now" is not an interval — must return null so the SQL generator emits
		 * the platform's current-timestamp function instead of an integer literal.
		 */
		public function testNowReturnsNull(): void {
			$this->assertNull(AstDate::tryParseInterval('now'));
		}
		...
		/**
		 * A plain string with no recognisable unit must return null.
		 */
		public function testUnrecognisedStringReturnsNull(): void {
			$this->assertNull(AstDate::tryParseInterval('yesterday'));
		}
		
		/**
		 * A bare number with no unit must return null.
		 */
		public function testBareNumberReturnsNull(): void {
			$this->assertNull(AstDate::tryParseInterval('42'));
		}
		
		/**
		 * An unknown unit must return null, not silently produce zero.
		 */
		public function testUnknownUnitReturnsNull(): void {
			$this->assertNull(AstDate::tryParseInterval('6 fortnights'));
		}
		
		/**
		 * Extra garbage tokens around valid pairs must cause null to be returned.
		 * e.g. "6 days ago" — "ago" is not a unit and must not be silently ignored.
		 */
		public function testExtraTokensReturnNull(): void {
			$this->assertNull(AstDate::tryParseInterval('6 days ago'));
		}
		
		public function testLeadingGarbageReturnsNull(): void {
			$this->assertNull(AstDate::tryParseInterval('about 6 days'));
		}
		
		/**
		 * An empty string must return null.
		 */
		public function testEmptyStringReturnsNull(): void {
			$this->assertNull(AstDate::tryParseInterval(''));
		}
