<?php
	...
	/**
	 * Publishes the PaypalExpress configuration file to the project's config directory.
	...
	class InitCommand extends CommandBase {
		...
		public function getDescription(): string {
			return "Publishes the PaypalExpress config file to config/paypal_express.php";
		}
		
		/**
		 * Copy the default PaypalExpress config file to the project's config directory.
		 * @param ConfigurationManager $config Configuration manager instance
		 * @return int Exit code (0 for success, 1 for failure)
		 */
		public function execute(ConfigurationManager $config): int {
			$source = dirname(__FILE__) . "/../../config/paypal_express.php";
			$target = ComposerUtils::getProjectRoot() . "/config/paypal_express.php";
			...
			if ($result) {
				$this->getOutput()->success("Published config/paypal_express.php");
			} else {
