<?php
	...
	/**
	 * Publishes the Handlebars configuration file to the project's config directory.
	...
	class InitCommand extends CommandBase {
		...
		public function getDescription(): string {
			return "Publishes the Handlebars config file to config/handlebars.php";
		}
		
		/**
		 * Copy the default Handlebars config file to the project's config directory.
		 * @param ConfigurationManager $config Configuration manager instance
		 * @return int Exit code (0 for success, 1 for failure)
		 */
		public function execute(ConfigurationManager $config): int {
			$source = dirname(__FILE__) . "/../../config/handlebars.php";
			$target = ComposerUtils::getProjectRoot() . "/config/handlebars.php";
			...
			if ($result) {
				$this->getOutput()->success("Published config/handlebars.php");
			} else {
