<?php
	...
	class RequestExtractor {
		 ...
		 * Regexp
		 */
		private const string SENSITIVE_HEADER_PATTERN = '/^(authorization|cookie|x-api-key|x-auth-token|x-access-token|bearer|php-auth-(user|pw|digest)|www-authenticate|proxy-authorization|x-forwarded-authorization|x-.*-(key|token|secret)|.*-auth.*)$/i';
		...
		private function getFilteredHeaders(): array {
			...
			foreach ($headers as $name => $values) {
				if (preg_match(self::SENSITIVE_HEADER_PATTERN, $name)) {
					continue;
				}
