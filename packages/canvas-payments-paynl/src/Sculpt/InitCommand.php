<?php
	...
	/**
	 * Publishes the PayNL configuration file to the project's config directory.
	...
	class InitCommand extends CommandBase {
		...
		public function getDescription(): string {
			return "Publishes the PayNL config file to config/paynl.php";
		}
		
		/**
		 * Copy the default PayNL config file to the project's config directory.
		 * @param ConfigurationManager $config Configuration manager instance
		 * @return int Exit code (0 for success, 1 for failure)
		 */
		public function execute(ConfigurationManager $config): int {
			$source = dirname(__FILE__) . "/../../config/paynl.php";
			$target = ComposerUtils::getProjectRoot() . "/config/paynl.php";
			...
			if ($result) {
				$this->getOutput()->success("Published config/paynl.php");
			} else {
