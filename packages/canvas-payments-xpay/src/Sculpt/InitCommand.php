<?php
	...
	namespace Quellabs\Payments\XPay\Sculpt;
	...
	/**
	 * Publishes the XPay configuration file to the project's config directory.
	...
	class InitCommand extends CommandBase {
		...
		public function getDescription(): string {
			return "Publishes the XPay config file to config/xpay.php";
		}
		
		/**
		 * Copy the default XPay config file to the project's config directory.
		 * @param ConfigurationManager $config Configuration manager instance
		 * @return int Exit code (0 for success, 1 for failure)
		 */
		public function execute(ConfigurationManager $config): int {
			$source = dirname(__FILE__) . "/../../config/xpay.php";
			$target = ComposerUtils::getProjectRoot() . "/config/xpay.php";
			...
			if ($result) {
				$this->getOutput()->success("Published config/xpay.php");
			} else {
