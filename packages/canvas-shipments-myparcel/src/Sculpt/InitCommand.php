<?php
	...
	namespace Quellabs\Shipments\MyParcel\Sculpt;
	...
	/**
	 * Publishes the MyParcel configuration file to the project's config directory.
	...
	class InitCommand extends CommandBase {
		...
		public function getDescription(): string {
			return "Publishes the MyParcel config file to config/myparcel.php";
		}
		
		/**
		 * Copy the default MyParcel config file to the project's config directory.
		 * @param ConfigurationManager $config Configuration manager instance
		 * @return int Exit code (0 for success, 1 for failure)
		 */
		public function execute(ConfigurationManager $config): int {
			$source = dirname(__FILE__) . "/../../config/myparcel.php";
			$target = ComposerUtils::getProjectRoot() . "/config/myparcel.php";
			...
			if ($result) {
				$this->getOutput()->success("Published config/myparcel.php");
			} else {
