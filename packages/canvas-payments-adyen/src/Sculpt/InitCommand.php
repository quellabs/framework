<?php
	...
	namespace Quellabs\Payments\Adyen\Sculpt;
	...
	/**
	 * Publishes the Adyen configuration file to the project's config directory.
	...
	class InitCommand extends CommandBase {
		...
		public function getDescription(): string {
			return "Publishes the Adyen config file to config/adyen.php";
		}
		
		/**
		 * Copy the default Adyen config file to the project's config directory.
		 * @param ConfigurationManager $config Configuration manager instance
		 * @return int Exit code (0 for success, 1 for failure)
		 */
		public function execute(ConfigurationManager $config): int {
			$source = dirname(__FILE__) . "/../../config/adyen.php";
			$target = ComposerUtils::getProjectRoot() . "/config/adyen.php";
			...
			if ($result) {
				$this->getOutput()->success("Published config/adyen.php");
			} else {
