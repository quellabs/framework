<?php
	...
	/**
	 * Publishes the Buckaroo configuration file to the project's config directory.
	...
	class InitCommand extends CommandBase {
		...
		public function getDescription(): string {
			return "Publishes the Buckaroo config file to config/buckaroo.php";
		}
		
		/**
		 * Copy the default Buckaroo config file to the project's config directory.
		 * @param ConfigurationManager $config Configuration manager instance
		 * @return int Exit code (0 for success, 1 for failure)
		 */
		public function execute(ConfigurationManager $config): int {
			$source = dirname(__FILE__) . "/../../config/buckaroo.php";
			$target = ComposerUtils::getProjectRoot() . "/config/buckaroo.php";
			...
			if ($result) {
				$this->getOutput()->success("Published config/buckaroo.php");
			} else {
