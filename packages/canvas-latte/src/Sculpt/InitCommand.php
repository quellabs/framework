<?php
	...
	/**
	 * Publishes the Latte configuration file to the project's config directory.
	...
	class InitCommand extends CommandBase {
		...
		public function getDescription(): string {
			return "Publishes the Latte config file to config/latte.php";
		}
		
		/**
		 * Copy the default Latte config file to the project's config directory.
		 * @param ConfigurationManager $config Configuration manager instance
		 * @return int Exit code (0 for success, 1 for failure)
		 */
		public function execute(ConfigurationManager $config): int {
			$source = dirname(__FILE__) . "/../../config/latte.php";
			$target = ComposerUtils::getProjectRoot() . "/config/latte.php";
			...
			if ($result) {
				$this->getOutput()->success("Published config/latte.php");
			} else {
