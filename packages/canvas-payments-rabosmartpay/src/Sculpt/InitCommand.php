<?php
	...
	/**
	 * Publishes the RaboSmartPay configuration file to the project's config directory.
	...
	class InitCommand extends CommandBase {
		...
		public function getDescription(): string {
			return "Publishes the RaboSmartPay config file to config/rabosmartpay.php";
		}
		
		/**
		 * Copy the default RaboSmartPay config file to the project's config directory.
		 * @param ConfigurationManager $config Configuration manager instance
		 * @return int Exit code (0 for success, 1 for failure)
		 */
		public function execute(ConfigurationManager $config): int {
			$source = dirname(__FILE__) . "/../../config/rabosmartpay.php";
			$target = ComposerUtils::getProjectRoot() . "/config/rabosmartpay.php";
			...
			if ($result) {
				$this->getOutput()->success("Published config/rabosmartpay.php");
			} else {
