<?php
	...
	/**
	 * Publishes the Blade configuration file to the project's config directory.
	...
	class InitCommand extends CommandBase {
		...
		public function getDescription(): string {
			return "Publishes the Blade config file to config/blade.php";
		}
		
		/**
		 * Copy the default Blade config file to the project's config directory.
		 * @param ConfigurationManager $config Configuration manager instance
		 * @return int Exit code (0 for success, 1 for failure)
		 */
		public function execute(ConfigurationManager $config): int {
			$source = dirname(__FILE__) . "/../../config/blade.php";
			$target = ComposerUtils::getProjectRoot() . "/config/blade.php";
			...
			if ($result) {
				$this->getOutput()->success("Published config/blade.php");
			} else {
