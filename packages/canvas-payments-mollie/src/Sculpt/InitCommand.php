<?php
	...
	namespace Quellabs\Payments\Mollie\Sculpt;
	...
	/**
	 * Publishes the Mollie configuration file to the project's config directory.
	...
	class InitCommand extends CommandBase {
		...
		public function getDescription(): string {
			return "Publishes the Mollie config file to config/mollie.php";
		}
		
		/**
		 * Copy the default Mollie config file to the project's config directory.
		 * @param ConfigurationManager $config Configuration manager instance
		 * @return int Exit code (0 for success, 1 for failure)
		 */
		public function execute(ConfigurationManager $config): int {
			$source = dirname(__FILE__) . "/../../config/mollie.php";
			$target = ComposerUtils::getProjectRoot() . "/config/mollie.php";
			...
			if ($result) {
				$this->getOutput()->success("Published config/mollie.php");
			} else {
