...
`$state->metadata['paymentIntentId']` — you'll need it as `RefundRequest::$transactionId`.
...
public function onPaymentExchange(PaymentState $state): void {
    if ($state->state === PaymentStatus::Paid) {
        $this->orderRepository->updatePaymentIntentId(
            $state->transactionId,
            $state->metadata['paymentIntentId']
        );
    }
}
...
$request = new RefundRequest(
    transactionId: $order->paymentIntentId,   // retrieved from your orders table
    paymentModule: 'stripe',
...
- **PaymentIntent ID** — available in `PaymentState::$metadata['paymentIntentId']` when a `PaymentStatus::Paid`
