...
ObjectQuel is a domain-level query engine with integrated ORM capabilities, built on the Data Mapper pattern. It
provides a powerful query language designed for expressing complex data relationships naturally, while offering clean
separation between entities and persistence logic. ObjectQuel combines its purpose-built query language with structured
data enrichment and is powered by CakePHP's robust database foundation under the hood.

## Why ObjectQuel?

ObjectQuel addresses fundamental challenges in working with relational data through its query-first architecture:
...
ObjectQuel provides a powerful query language for expressing complex data relationships:
...
// 3. Query using ObjectQuel language - express complex relationships naturally
...
foreach($results as $row) {
    echo $row['p']->getName() . ' - ' . $row['categoryName'];
}

// 4. Standard ORM operations are also available
...
ObjectQuel's architecture centers around its query engine, with integrated ORM capabilities:

**Query Engine Components:**
- **ObjectQuel Parser**: Handles reading and parsing of the ObjectQuel query language
...
**ORM Components:**
- **EntityManager**: Central coordinator for both query execution and entity management
...
ObjectQuel provides multiple ways to retrieve data, with the query language as the primary approach for complex queries:

#### 1. Using ObjectQuel Query Language (Recommended for Complex Queries)

```php
// Express complex relationships and conditions naturally
$results = $entityManager->executeQuery("
    range of p is App\\Entity\\ProductEntity
    range of c is App\\Entity\\CategoryEntity via p.categories
    retrieve (p, c.name as categoryName)
    where p.productId = :productId and c.active = true
", [
    'productId' => 1525
]);

foreach($results as $row) {
    echo $row['p']->getName() . ' in ' . $row['categoryName'];
}
```

#### 2. Using `find()` (Simple Primary Key Lookups)
...
#### 3. Using `findBy()` (Simple Criteria Matching)

```php
// Find entities matching simple criteria
